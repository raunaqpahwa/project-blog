            ...
            {range(numOfGroups).map((groupIndex) => (
              <div key={groupIndex} className={styles.group}>
                {range(numOfItemsPerGroup).map((index) => {
                  return (
                    <motion.div
                      layoutId={`circle-${groupIndex * numOfItemsPerGroup + index}`}
                      key={`circle-${groupIndex * numOfItemsPerGroup + index}`}
        ...
        {includeRemainderArea && (
          <div className={styles.remainderArea}>
            ...
            {range(remainder).map((index) => {
              return (
                <motion.div
                  layoutId={`circle-${numOfGroups * numOfItemsPerGroup + (remainder - index - 1)}`}
                  key={`circle-${numOfGroups * numOfItemsPerGroup + (remainder - index - 1)}`}
                  className={styles.item}
                />
              );
            })}
